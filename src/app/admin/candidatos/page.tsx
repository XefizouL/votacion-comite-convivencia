  ...
  return (
    <main className="min-h-screen bg-slate-50 p-8">
      <div className="max-w-6xl mx-auto bg-wp-white rounded-3xl border border-black/10 shadow-lg p-8">
        <h1 className="text-3xl font-bold text-wp-black mb-2">Administración de Candidatos</h1>
        <p className="text-wp-black/70 mb-8">Aprueba o rechaza las postulaciones (Máximo 4 candidatos oficiales).</p>

        <div className="grid gap-6 mb-8 md:grid-cols-3">
          <div className="rounded-3xl border border-black/10 bg-slate-100 p-6">
            <p className="text-sm uppercase tracking-[0.25em] text-wp-primary font-semibold mb-2">Inicio</p>
            <p className="text-2xl font-bold text-wp-black">{openDate}</p>
          </div>
          <div className="rounded-3xl border border-black/10 bg-slate-100 p-6">
            <p className="text-sm uppercase tracking-[0.25em] text-wp-primary font-semibold mb-2">Cierre</p>
            <p className="text-2xl font-bold text-wp-black">{closeDate}</p>
          </div>
          <div className="rounded-3xl border border-black/10 bg-slate-100 p-6">
            <p className="text-sm uppercase tracking-[0.25em] text-wp-primary font-semibold mb-2">Firmas</p>
            <p className="text-wp-black">Director ejecutivo</p>
            <p className="text-wp-black">Área SST</p>
          </div>
        </div>
        ...
        <div className="bg-wp-white rounded-xl shadow-sm border border-black overflow-hidden">
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {candidates.length === 0 && (
                <tr>
                  <td colSpan={4} className="p-8 text-center text-wp-black/70">No hay candidatos postulados aún.</td>
                </tr>
              )}
              {candidates.map((candidate) => (
                <tr key={candidate.id} className="border-b hover:bg-wp-primary/10">
                  <td className="p-4">
                    ...
                    <img src={candidate.photoUrl} alt="Foto" className="w-12 h-12 rounded-full object-cover border border-black" />
                  </td>
                  <td className="p-4 font-medium text-wp-black">{candidate.fullName}</td>
                  <td className="p-4">
                  ...
                  <td className="p-4 text-center space-x-2">
                    {candidate.status === 'PENDING' && (
                      <>
                        {/* Botón Aprobar */}
                        <form action={approveCandidateAction.bind(null, candidate.id)} className="inline">
                        ...
                        {/* Botón Rechazar */}
                        <form action={rejectCandidateAction.bind(null, candidate.id)} className="inline">
